...
class MannualStrategy(object):
    ...
    def manualrule_portfolio(self, symbol, sd, ed, sv, commission=9.95, impact=0.005):
        dates = pd.date_range(sd, ed)
        symbols = []
        symbols.append(symbol)
...
if __name__ == "__main__":
    ms = MannualStrategy(symbol = "JPM", sd = dt.datetime(2010, 1, 1), ed = dt.datetime(2011, 12, 31), sv = 100000)
    df_trades = ms.testPolicy(symbol = "JPM", sd = dt.datetime(2010, 1, 1), ed = dt.datetime(2011, 12, 31), sv = 100000)
    portval_ms = ms.manualrule_portfolio(symbol = "JPM", sd = dt.datetime(2010, 1, 1), ed = dt.datetime(2011, 12, 31), sv = 100000, commission=9.95, impact=0.005)
    portval_benchmark = ms.benchmark_portfolio(symbol = "JPM", sd = dt.datetime(2010, 1, 1), ed = dt.datetime(2011,12, 31), sv = 100000, commission=9.95, impact=0.005)
    ax = portval_ms.plot(title="Benchmark vs. Manual Strategy (Out of Sample)", color = "black", label = "Manual")
