...
class theoreticallyOptimalStrategy(object):
    ...
    def get_tos_portfolio(self,symbol,sd,ed,sv,commission=0,impact=0):
        ...
        print "Cumulative Return of {}: {}".format("Theoretical Optimal Strategy", cr_tos)
        print "Standard Deviation of daily return of {}: {}".format("Theoretical Optimal Strategy", sdr_tos)
        print "Mean Daily Return of {}: {}".format("Theoretical Optimal Strategy", adr_tos)
...
if __name__ == "__main__":
    ...
    portval_tos = tos.get_tos_portfolio(symbol="JPM", sd=dt.datetime(2008,1,1), ed=dt.datetime(2009,12,31), sv=100000, commission=0, impact=0)
    portval_benchmark = tos.get_benchmark_portfolio(symbol="JPM", sd=dt.datetime(2008,1,1), ed=dt.datetime(2009,12,31), sv=100000, commission=0, impact=0)
    ax = portval_tos.plot(title="Benchmark vs. Theoretically Optimal Strategy", fontsize=12, color = "black",label = "Optimal Strategy")
    portval_benchmark.plot(ax = ax, color = "blue", label = "Benchmark")
    ax.set_xlabel('Dates')
    ax.set_ylabel('Normalized Value')
    plt.grid(True)
    plt.legend(loc=0)
    plt.savefig("Benchmark vs. Theoretically Optimal Strategy.png")
    plt.show()
